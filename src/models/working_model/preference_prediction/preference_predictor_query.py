...
index = 0
VERSION = 'Traj_dS103_Query_dS005-104'

#dir
DIR_PREDICTION_ROOT = os.getcwd() # the script dir
# For simulation data-----------
DIR_PREDICTION_DATA_TRAJECTORY = os.path.join(DIR_PREDICTION_ROOT,'..','..',\
                                                 'data','data_dynamic','dS103')

DIR_PREDICTION_DATA_QUERY_STATE = os.path.join(DIR_PREDICTION_ROOT,'..','..',\
                                                   'data','data_dynamic','dS00'+str(index)+'_pred')
DIR_MODEL = 'test_on_simulation_data/training_result/caches/cache_dS103_v1_commit_'
...
#  DIR_MODEL = 'test_on_human_data/training_result/caches/cache_S030_v9_commit_78092b'
  ...
  def parse_prediction_data_whole_data_set(self):
    ...
    preference_predictor.parse_prediction_data_trajectory(directory = DIR_PREDICTION_DATA_TRAJECTORY,\
    ...
    if self.WITH_PREDNET:
      ...
      preference_predictor.parse_prediction_data_query_state(directory = DIR_PREDICTION_DATA_QUERY_STATE,\
                                                             subset_size = preference_predictor.SUBSET_SIZE,\
...
#                                                         all_tensors=True)
  ...
  def save_predictions(self):
    ...
    file_name_final_target_predictions = os.path.join(self.DIR_MODEL_PREDICTION_RESULT_THIS_VERSION,\
                                                      'final_target_predictions_dS00'+str(index)+'.csv')
    ...
    file_name_final_sum_predictions = os.path.join(self.DIR_MODEL_PREDICTION_RESULT_THIS_VERSION,\
                                                      'final_sum_predictions_dS00'+str(index)+'.csv')
...
if __name__ == "__main__":
    ...
    for index in range(START,END+1):
        ...
        VERSION = 'Traj_dS103_Query_dS00'+str(index)+'_pred'
        DIR_PREDICTION_DATA_QUERY_STATE = os.path.join(DIR_PREDICTION_ROOT,'..','..',\
                                                'data','data_dynamic','dS00'+str(index)+'_pred')
        DIR_MODEL_PREDICTION_RESULT_ROOT = os.path.join(DIR_MODEL,'prediction')
        DIR_MODEL_PREDICTION_RESULT_THIS_VERSION = \
