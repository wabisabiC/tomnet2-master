...
def startmaze():
    ...
    return x_random,y_random,GAME_ART,social_reward

#agent start place, distance and energy
def inputs(x_random,y_random,agent_place,social_reward):
    #place
    if agent_place == []:    
        for i in range(n_chosen_agents):    
...
while simulation_time < SIMULATION_TOTAL:
    x_random,y_random,GAME_ART,social_reward=startmaze()
    while path_step < step_total:   
        agent_place,distance,force=inputs(x_random,y_random,agent_place,social_reward)
